...
| リクエスト相関 | 全メッセージが`requestId`（string、UUID）を持つ（後述） |

**`requestId`**：Gate側が要求ごとに新規発行するUUID文字列。**この節で定義する全メッセージ**（`evacuate-request`/`evacuate-complete`を含む）が`requestId`フィールドを持ち、Managerは要求に対応する応答（`*-rejected`・`*-failed`・`hardcore-ready`・`deactivate-complete`・`state-response`等）に、受け取った`requestId`をそのままエコーバックする。同一コネクション上で複数プレイヤーの操作が絡む場合に、「どの応答がどの要求に対応するか」をメッセージ内容や到着順に頼らず一意に判別できるようにするためのもの（`architecture-gate.md`の旧記述「メッセージ内容や送信順で相関を取る簡易実装」を置き換える）。Manager側は値の中身を一切解釈せず、受け取った文字列をそのまま運んで返すだけの不透明な値として扱う。
...
| `start-failed` | Manager → Gate | `start`を受理した後、退避・プロセス停止・ワールド準備・プロセス起動・`ready`待ちのいずれかで失敗した場合 |
| `load-failed` | Manager → Gate | `load`を受理した後、同上のいずれかで失敗した場合 |
| `deactivate-failed` | Manager → Gate | `deactivate`を受理した後、退避・プロセス停止のいずれかで失敗した場合 |
...
`state-query`（Gate → Manager）:

| フィールド | 型 | 必須 | 説明 |
|---|---|---|---|
| `type` | string | ✓ | 固定値 `"state-query"` |
| `requestId` | string | ✓ | UUID（1節） |

```json
{"type":"state-query","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000001"}
```
...
| `requestId` | string | ✓ | 対応する`state-query`の値をそのままエコー（1節） |
...
```json
{"type":"state-response","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000001","state":"ready","running":"true"}
```
...
| `requestId` | string | ✓ | 完了した`start`/`load`要求の値をそのままエコー（1節） |

```json
{"type":"hardcore-ready","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000002"}
```
...
```json
{"type":"start","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000002","clean":false,"requestedBy":"Steve"}
```
...
```json
{"type":"load","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000003","name":"latest","force":false,"requestedBy":"Steve"}
```
...
```json
{"type":"deactivate","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000004","requestedBy":"Steve"}
```
...
| `requestId` | string | ✓ | 対応する`start`/`load`/`deactivate`の値をそのままエコー（1節） |
| `reason` | string | ✓ | 拒否理由（例：`"既に起動しています"`、`"挑戦が進行中です"`、`"アーカイブ<name>は存在しません"`、`"既に停止しています"`、`"処理中です。しばらくお待ちください"`） |

```json
{"type":"load-rejected","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000003","reason":"アーカイブ save1 は存在しません"}
```
```json
{"type":"start-rejected","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000002","reason":"既に起動しています"}
```
...
```json
{"type":"evacuate-request","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000002","reason":"reset"}
```

`evacuate-complete`（Gate → Manager）:

| フィールド | 型 | 必須 | 説明 |
|---|---|---|---|
| `type` | string | ✓ | 固定値 `"evacuate-complete"` |
| `requestId` | string | ✓ | 対応する`evacuate-request`の値をそのままエコー（1節） |

```json
{"type":"evacuate-complete","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000002"}
```
...
| `requestId` | string | ✓ | 対応する`deactivate`の値をそのままエコー（1節） |

```json
{"type":"deactivate-complete","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000004"}
```

Gateはこれを受けて、`/deactivate`実行者へ「サーバーを停止しました」と表示する。

### 3.5b `start-failed` / `load-failed` / `deactivate-failed`

Manager → Gate。`start`/`load`/`deactivate`を**受理した後**（＝`*-rejected`にはならなかった後）、退避・プロセス停止・ワールド準備・プロセス起動・`ready`待ちのいずれかで失敗した場合の1回限りの通知。`hardcore-ready`/`deactivate-complete`（成功）の失敗版に相当する。

**背景**：`*-rejected`は受理する**前**のチェック（プロセス状態・`running`・アーカイブ存在）専用であり、受理した後の失敗（例：hardcoreプロセスがファイル権限エラーで即座にクラッシュした）を伝える手段が当初存在しなかった。この場合Gate側は`evacuate-request`/`evacuate-complete`のやり取りまでは完了するが、その後`hardcore-ready`が来ないまま無期限に待ち続ける、という不具合が実機で見つかった（`specification.md` 2.1節「受理後に失敗した場合」参照）。

| フィールド | 型 | 必須 | 説明 |
|---|---|---|---|
| `type` | string | ✓ | `"start-failed"` \| `"load-failed"` \| `"deactivate-failed"` |
| `requestId` | string | ✓ | 対応する`start`/`load`/`deactivate`の値をそのままエコー（1節） |
| `reason` | string | ✓ | 失敗理由（人間可読の文字列。例：`"process exited before ready"`、`"stop timed out"`） |
| `recovered` | bool | ✓ | Managerが`Process.IsRunning()`でプロセスの生死を確認できた（＝生きていないことを確認できた）場合に`true`。この場合プロセス状態は`停止中`へ戻され、`/start`等を即座に再試行できる。確認できなかった場合は`false`——プロセス状態は`起動処理中`/`停止処理中`のまま維持され、`/start`・`/start clean`・`/load`・`/deactivate`のいずれも遷移中ガード（3.4節）により「処理中です」で拒否され続ける |

```json
{"type":"start-failed","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000002","reason":"process exited before ready","recovered":true}
```
```json
{"type":"load-failed","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000003","reason":"stop timed out","recovered":false}
```

`recovered:false`の場合、Manager自体を再起動する以外に現状復旧手段が無い（`phase`は永続化されないため再起動で`stopped`に戻る。`specification.md` 10節の未決事項参照）。Gateは`reason`に加え、`recovered:false`の場合は「手動での確認が必要です」等、再試行だけでは解決しない可能性がある旨も表示すべき。

### 3.6 `savedata-query` / `savedata-response`

| メッセージ | フィールド | 型 | 必須 | 説明 |
|---|---|---|---|---|
| `savedata-query` | `type` | string | ✓ | 固定値 `"savedata-query"` |
| | `requestId` | string | ✓ | UUID（1節） |
| `savedata-response` | `type` | string | ✓ | 固定値 `"savedata-response"` |
| | `requestId` | string | ✓ | 対応する`savedata-query`の値をそのままエコー（1節） |
...
```json
{"type":"savedata-query","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000005"}
```
```json
{"type":"savedata-response","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000005","events":[
...
| | `requestId` | string | ✓ | 対応する`senpan-query`の値をそのままエコー（1節） |
| | `mode` | string | ✓ | 要求と同じ値をエコーする |
| | `entries` | array | ✓ | `mode=list`：`{"player":{"uuid","name"},"count":n}`の配列。`mode=count`：プレイヤーごとの回数のみ（表示形式はGate側で整形） |

```json
{"type":"senpan-query","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000006","mode":"count"}
```
```json
{"type":"senpan-response","requestId":"b1e6c0b0-0a1a-4b1a-9b1a-000000000006","mode":"count","entries":[
...
退避を伴う、破壊的な系統。以下の図では`requestId`を`...`に含めて省略している（全メッセージに付与される点は1節参照）。
...
sequenceDiagram
    ...
    alt 退避後、プロセス停止・world/削除・プロセス起動・ready待ちのいずれかで失敗
        M->>G: {"type":"start-failed","reason":"...","recovered":...}
        G->>P: 失敗理由を表示
    else 成功
...
sequenceDiagram
    ...
    alt runningがtrue、またはアーカイブが存在しない（拒否）
        ...
        G->>G: hardcore接続中の全プレイヤーをlobbyへ強制転送（プロセス停止中なら不要）
        G->>M: {"type":"evacuate-complete"}
        alt 退避後、プロセス停止・復元・プロセス起動・ready待ちのいずれかで失敗
            M->>G: {"type":"load-failed","reason":"...","recovered":...}
            G->>P: 失敗理由を表示
        else 成功
...
sequenceDiagram
    ...
    alt 既に停止中/停止処理中（拒否）
        ...
        G->>M: {"type":"evacuate-complete"}
        alt プロセス停止に失敗
            M->>G: {"type":"deactivate-failed","reason":"...","recovered":...}
            G->>P: 失敗理由を表示
        else 成功
            M->>H: プロセス正常停止
...
- `start-failed`/`load-failed`/`deactivate-failed`が`recovered:false`を返した場合の手動復旧手段（Manager再起動以外の手段を用意するか、`specification.md` 10節）
